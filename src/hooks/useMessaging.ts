...
/**
 * Unified messaging hook that handles conversations and messages
 */
export const useMessaging = () => {
  const queryClient = useQueryClient();

  // Real-time subscription for all messaging data
  useEffect(() => {
      ...
      const channel = supabase
        ...
        .on(
          'postgres_changes',
          {
            ...
            schema: 'public',
            table: 'conversations',
            filter: conversationIds.length > 0 ? `id=in.(${conversationIds.join(',')})` : 'id=eq.never'
          },
          () => {
            queryClient.invalidateQueries({ queryKey: ['conversations'] });
          }
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const transformedConversations: Conversation[] = userConversations?.map((conv: any) => {
          ...
          return {
            id: conv.id,
            title: conv.title || conv.title || 'Direct Message',
