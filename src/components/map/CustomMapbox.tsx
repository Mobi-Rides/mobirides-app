...
const CustomMapbox = ({
  ...
  useEffect(() => {
    ...
    const newMarkers = onlineHosts.map(host => {
      ...
      const marker = new mapboxgl.Marker(el).setLngLat([host.longitude, host.latitude]).setPopup(new mapboxgl.Popup({
        offset: 25
      }).setHTML(`<p class="font-medium">${host.full_name || 'Host'}</p>`)).addTo(map.current!);
      return marker;
  ...
  return <div className="relative w-full h-full bottom-0 left-0 right-0 top-0">
      ...
      <Dpad onUp={onUp} onDown={onDown} onLeft={onLeft} onRight={onRight} onReset={onReset} />
    </div>;
};
