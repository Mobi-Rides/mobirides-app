                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  {(selectedClaim.status === 'submitted' || selectedClaim.status === 'under_review') && (
                    <button
                      onClick={() => {
                        const notes = (document.getElementById('review-notes') as HTMLTextAreaElement)?.value;
                        updateClaimStatus(selectedClaim.id, 'more_info_needed', notes);
                        // Log activity for info request
                        InsuranceService.logClaimActivity(
                          selectedClaim.id,
                          'info_requested',
                          notes || 'Additional information requested from claimant',
                          'Admin'
                        );
                        // Notify renter via toast (notification service would handle actual notification)
                        alert(`Info request sent to renter. They will be notified to submit additional documents.`);
                      }}
