  ...
  const commonBenefits = [
    {
      key: 'coverage_cap',
      label: 'Coverage Cap',
      format: (v: number | null) => v ? `P ${v.toLocaleString()}` : 'N/A',
      tooltip: 'The maximum amount the insurance will pay out for a claim.'
    },
    {
      key: 'excess',
      label: 'Excess',
      format: (v: number | null, pkg: PremiumCalculation) => {
        if (pkg.excessPercentage !== null) {
          return `${pkg.excessPercentage * 100}% of claim`;
        }
        return v ? `P ${v.toLocaleString()}` : 'N/A';
      },
      tooltip: 'The amount you pay towards a claim before the insurance covers the rest.'
    },
                  ...
                  <CardContent className="space-y-4">
                                ...
                                <TooltipProvider>
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <HelpCircle className="ml-1 inline-block h-3 w-3 text-muted-foreground cursor-help" />
                                    </TooltipTrigger>
                                    <TooltipContent>
                                      <p>{benefit.tooltip}</p>
                                    </TooltipContent>
                    ...
                    {pkg.exclusions && pkg.exclusions.length > 0 && (
                      <div className="pt-2 border-t">
                        ...
                        <ul className="space-y-1">
                          {pkg.exclusions.slice(0, 3).map((exclusion, i) => (
