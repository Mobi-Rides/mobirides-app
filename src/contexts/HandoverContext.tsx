...
interface HandoverContextType {
  ...
  isLoading: boolean;
  isHost: boolean;
  updateLocation: (location: Omit<HandoverLocation, "timestamp">) => Promise<boolean>;
...
export const HandoverProvider: React.FC<HandoverProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (handoverData) {
      ...
      const convertedData = {
        ...handoverData,
        host_location: handoverData.host_location ? handoverData.host_location as unknown as HandoverLocation : null,
        renter_location: handoverData.renter_location ? handoverData.renter_location as unknown as HandoverLocation : null
      };
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToHandoverUpdates(
      ...
      (updatedHandover) => {
        ...
        const convertedData = {
          ...updatedHandover,
          host_location: updatedHandover.host_location ? updatedHandover.host_location as unknown as HandoverLocation : null,
          renter_location: updatedHandover.renter_location ? updatedHandover.renter_location as unknown as HandoverLocation : null
        };
        setHandoverStatus(convertedData);
      }
    );
  ...
  // Update location
  const updateLocation = async (location: Omit<HandoverLocation, "timestamp">) => {
