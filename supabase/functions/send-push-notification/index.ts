...
// Whitelist of allowed push service domains to prevent SSRF
  ...
  try {
    ...
    if (!isAllowedPushEndpoint(subscription.endpoint)) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "Push endpoint domain not allowed",
        }),
        {
          status: 403,
...
async function sendWebPushNotification(
  ...
  if (!response.ok) {
    ...
    console.error(
      `Push service error: ${response.status} ${response.statusText} - ${errorText}`
    );
    throw new Error(
      `Push service error: ${response.status} ${response.statusText} - ${errorText}`
    );
