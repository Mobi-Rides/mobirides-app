-- Fix remaining handover notification functions that still reference pickup_location
-- These functions were not covered in the previous fix migration

-- Drop and recreate create_handover_step_notification function (the one with handover_session_id parameter)
...
-- Drop and recreate create_handover_progress_notification function (the one with handover_session_id parameter)
...
-- Add comments
COMMENT ON FUNCTION public.create_handover_step_notification(uuid, text, uuid) IS 'Creates notifications when handover steps are completed (fixed pickup_location issue)';
COMMENT ON FUNCTION public.create_handover_progress_notification(uuid) IS 'Creates progress notifications for handover sessions (fixed pickup_location issue)';