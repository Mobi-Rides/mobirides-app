...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'user_verifications') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.user_verifications;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'verification_documents') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.verification_documents;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'phone_verifications') THEN
