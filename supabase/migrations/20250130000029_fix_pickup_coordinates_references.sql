-- Fix pickup_latitude and pickup_longitude column references in handover notification functions
-- The bookings table has 'latitude' and 'longitude' columns, not 'pickup_latitude' and 'pickup_longitude'

-- Drop and recreate the create_handover_step_notification function with correct column names
...
BEGIN
    ...
    ELSIF session_record.latitude IS NOT NULL AND session_record.longitude IS NOT NULL THEN
        location_text := format('Lat: %s, Lng: %s', session_record.latitude, session_record.longitude);
    ELSE
...
-- Drop and recreate the create_handover_progress_notification function with correct column names
...
-- Add comments
COMMENT ON FUNCTION public.create_handover_step_notification(uuid, text, uuid) IS 'Creates notifications when handover steps are completed - fixed coordinate column references';
COMMENT ON FUNCTION public.create_handover_progress_notification(uuid) IS 'Creates progress notifications for handover sessions - fixed coordinate column references';