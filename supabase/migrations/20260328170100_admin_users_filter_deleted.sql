-- S9-010 / MOB-133: Filter soft-deleted users from admin user list
-- Adds show_deleted param (default false) to get_admin_users_complete RPC

DROP FUNCTION IF EXISTS public.get_admin_users_complete();

CREATE FUNCTION public.get_admin_users_complete(show_deleted BOOLEAN DEFAULT false)
RETURNS TABLE (
  id UUID,
  full_name TEXT,
  email TEXT,
