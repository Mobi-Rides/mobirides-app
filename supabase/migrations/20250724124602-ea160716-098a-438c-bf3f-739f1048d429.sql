-- Create RLS policies for handover_sessions table
...
-- Create RLS policies for handover_step_completion table
...
USING (
  EXISTS (
    ...
    AND (
      auth.uid() IN (
        SELECT user_id FROM bookings WHERE id = handover_sessions.booking_id
        UNION
        SELECT profiles.id FROM profiles 
        JOIN cars ON profiles.id = cars.owner_id 
        JOIN bookings ON cars.id = bookings.car_id 
...
-- Create RLS policies for identity_verification_checks table
