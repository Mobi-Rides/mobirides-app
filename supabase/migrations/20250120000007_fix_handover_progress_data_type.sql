-- Fix data type mismatch in handover notification functions
-- The issue: calculate_handover_progress returns JSONB but functions expect INTEGER
-- Solution: Extract progress_percentage from JSONB and cast to INTEGER

-- Drop and recreate create_handover_step_notification function with correct data type handling
...
BEGIN
    ...
    progress_percentage := (progress_result->>'progress_percentage')::INTEGER;
...
-- Drop and recreate create_handover_progress_notification function with correct data type handling
...
-- Add comments
COMMENT ON FUNCTION public.create_handover_step_notification(uuid, text, uuid) IS 'Creates notifications when handover steps are completed (fixed JSONB to INTEGER data type issue)';
COMMENT ON FUNCTION public.create_handover_progress_notification(uuid) IS 'Creates progress notifications for handover sessions (fixed JSONB to INTEGER data type issue)';