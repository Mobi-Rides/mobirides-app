drop trigger if exists "trigger_check_circular_reply" on "archive"."messages";

drop trigger if exists "update_messages_updated_at" on "archive"."messages";

drop trigger if exists "log_notification_trigger" on "archive"."notifications_backup";
...
drop trigger if exists "trigger_check_circular_reply_conversation" on "public"."conversation_messages";

drop trigger if exists "trigger_update_message_delivery_status" on "public"."conversation_messages";
...
drop trigger if exists "update_identity_keys_updated_at" on "public"."identity_keys";
...
drop trigger if exists "update_provider_health_metrics_updated_at" on "public"."provider_health_metrics";
...
drop trigger if exists "update_signal_sessions_updated_at" on "public"."signal_sessions";

drop trigger if exists "update_user_guide_progress_updated_at" on "public"."user_guide_progress";

drop trigger if exists "trigger_update_user_public_keys_updated_at" on "public"."user_public_keys";
...
drop trigger if exists "trigger_log_bypass_session_creation" on "public"."verification_bypass_sessions";

drop trigger if exists "trigger_log_bypass_session_deactivation" on "public"."verification_bypass_sessions";

drop policy "Admins can view all messages" on "archive"."messages";
...
drop policy "Super admins can view audit logs" on "public"."audit_logs";

drop policy "Hosts can update extension status" on "public"."booking_extensions";

drop policy "Users can view extensions for their bookings" on "public"."booking_extensions";

drop policy "Car owners can view bookings for their cars" on "public"."bookings";
...
drop policy "Users can update bookings they are involved with" on "public"."bookings";
...
drop policy "Users can add images to their own cars" on "public"."car_images";

drop policy "Users can delete their own car images" on "public"."car_images";

drop policy "Admins can update all cars" on "public"."cars";

drop policy "Super admins can update any car" on "public"."cars";

drop policy "Only admins can delete commission rates" on "public"."commission_rates";
...
drop policy "Admins can read dynamic pricing rules" on "public"."dynamic_pricing_rules";

drop policy "Super admins can manage dynamic pricing rules" on "public"."dynamic_pricing_rules";

drop policy "Users can access file encryption data for their messages" on "public"."file_encryption";
...
drop policy "Only admins can manage insurance commission rates" on "public"."insurance_commission_rates";
...
drop policy "Hosts can view host-only notifications" on "public"."notifications";

drop policy "Renters can view renter-only notifications" on "public"."notifications";

drop policy "Users can view system-wide notifications" on "public"."notifications";
...
drop policy "Admins can read platform settings" on "public"."platform_settings";

drop policy "Super admins can manage platform settings" on "public"."platform_settings";
...
drop policy "Booking participants can view locations" on "public"."real_time_locations";
...
drop policy "Admins can read all consents" on "public"."user_consents";
...
drop policy "Hosts can view their own transactions" on "public"."wallet_transactions";

drop policy "Users can create wallet transactions" on "public"."wallet_transactions";

drop policy "Users can view their own wallet transactions" on "public"."wallet_transactions";

drop policy "Users can view their wallet transactions" on "public"."wallet_transactions";
...
alter table "archive"."notifications_backup" drop constraint "notifications_related_booking_id_fkey";

alter table "archive"."notifications_backup" drop constraint "notifications_related_car_id_fkey";

alter table "archive"."notifications_backup" drop constraint "notifications_user_id_fkey";
...
alter table "public"."booking_extensions" drop constraint "booking_extensions_approved_by_fkey";

alter table "public"."booking_extensions" drop constraint "booking_extensions_booking_id_fkey";

alter table "public"."booking_extensions" drop constraint "booking_extensions_requested_by_fkey";
...
alter table "public"."file_encryption" drop constraint "file_encryption_message_id_fkey";
...
alter table "public"."notifications" drop constraint "notifications_related_booking_id_fkey";

alter table "public"."notifications" drop constraint "notifications_related_car_id_fkey";
...
alter table "public"."verification_bypass_logs" drop constraint "verification_bypass_logs_session_id_fkey";
...
drop function if exists "public"."create_booking_notification"(p_booking_id uuid, p_renter_notification_type notification_type, p_host_notification_type notification_type, p_title text, p_description text, p_metadata jsonb);

drop function if exists "public"."create_booking_notification"(p_booking_id uuid, p_renter_notification_type notification_type, p_host_notification_type notification_type, p_title text, p_description text, p_metadata jsonb, p_role_target notification_role);
...
drop function if exists "public"."create_wallet_notification"(p_wallet_transaction_id uuid, p_user_id uuid, p_type notification_type, p_title text, p_description text, p_metadata jsonb, p_role_target notification_role);
...
-- drop type "public"."http_response";
  ...
  create table "public"."system_broadcast_logs" (
...
alter table "public"."system_broadcast_logs" enable row level security;
...
alter table "archive"."messages_backup_20250930_093926" alter column "status" set data type public.message_status using "status"::text::public.message_status;

alter table "archive"."notifications_backup" alter column "type" set data type public.old_notification_type using "type"::text::public.old_notification_type;
...
CREATE UNIQUE INDEX system_broadcast_logs_pkey ON public.system_broadcast_logs USING btree (id);

CREATE UNIQUE INDEX system_broadcasts_pkey ON public.system_broadcasts USING btree (id);

alter table "public"."system_broadcast_logs" add constraint "system_broadcast_logs_pkey" PRIMARY KEY using index "system_broadcast_logs_pkey";

alter table "public"."system_broadcasts" add constraint "system_broadcasts_pkey" PRIMARY KEY using index "system_broadcasts_pkey";

alter table "public"."system_broadcast_logs" add constraint "system_broadcast_logs_broadcast_id_fkey" FOREIGN KEY (broadcast_id) REFERENCES public.system_broadcasts(id) ON DELETE CASCADE not valid;

alter table "public"."system_broadcast_logs" validate constraint "system_broadcast_logs_broadcast_id_fkey";

alter table "public"."system_broadcast_logs" add constraint "system_broadcast_logs_status_check" CHECK ((status = ANY (ARRAY['sent'::text, 'failed'::text]))) not valid;

alter table "public"."system_broadcast_logs" validate constraint "system_broadcast_logs_status_check";

alter table "public"."system_broadcast_logs" add constraint "system_broadcast_logs_user_id_fkey" FOREIGN KEY (user_id) REFERENCES auth.users(id) not valid;

alter table "public"."system_broadcast_logs" validate constraint "system_broadcast_logs_user_id_fkey";

alter table "public"."system_broadcasts" add constraint "system_broadcasts_audience_check" CHECK ((audience = ANY (ARRAY['all'::text, 'renters'::text, 'hosts'::text, 'verified'::text, 'active_30d'::text]))) not valid;

alter table "public"."system_broadcasts" validate constraint "system_broadcasts_audience_check";

alter table "public"."system_broadcasts" add constraint "system_broadcasts_channel_check" CHECK ((channel = ANY (ARRAY['email'::text, 'push'::text, 'both'::text]))) not valid;

alter table "public"."system_broadcasts" validate constraint "system_broadcasts_channel_check";

alter table "public"."system_broadcasts" add constraint "system_broadcasts_created_by_fkey" FOREIGN KEY (created_by) REFERENCES auth.users(id) not valid;

alter table "public"."system_broadcasts" validate constraint "system_broadcasts_created_by_fkey";

alter table "public"."system_broadcasts" add constraint "system_broadcasts_status_check" CHECK ((status = ANY (ARRAY['pending'::text, 'scheduled'::text, 'sending'::text, 'completed'::text, 'failed'::text, 'cancelled'::text]))) not valid;

alter table "public"."system_broadcasts" validate constraint "system_broadcasts_status_check";
...
alter table "public"."booking_extensions" add constraint "booking_extensions_approved_by_fkey" FOREIGN KEY (approved_by) REFERENCES public.profiles(id) not valid;

alter table "public"."booking_extensions" validate constraint "booking_extensions_approved_by_fkey";

alter table "public"."booking_extensions" add constraint "booking_extensions_booking_id_fkey" FOREIGN KEY (booking_id) REFERENCES public.bookings(id) ON DELETE CASCADE not valid;

alter table "public"."booking_extensions" validate constraint "booking_extensions_booking_id_fkey";

alter table "public"."booking_extensions" add constraint "booking_extensions_requested_by_fkey" FOREIGN KEY (requested_by) REFERENCES public.profiles(id) ON DELETE CASCADE not valid;

alter table "public"."booking_extensions" validate constraint "booking_extensions_requested_by_fkey";
...
CREATE OR REPLACE FUNCTION public.get_admin_users_complete(show_deleted boolean DEFAULT false)
 RETURNS TABLE(id uuid, full_name text, email text, role text, avatar_url text, phone_number text, created_at timestamp with time zone, updated_at timestamp with time zone, verification_status text, is_active boolean, is_deleted boolean, user_roles text[], is_restricted boolean, active_restrictions jsonb, vehicles_count bigint, bookings_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.suspend_user(p_user_id uuid, p_reason text, p_duration interval DEFAULT '7 days'::interval)
 RETURNS public.user_restrictions
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.transfer_vehicle(p_vehicle_id uuid, p_from_owner_id uuid, p_to_owner_id uuid, p_reason text DEFAULT NULL::text, p_notes text DEFAULT NULL::text)
...
grant delete on table "public"."system_broadcast_logs" to "anon";

grant insert on table "public"."system_broadcast_logs" to "anon";

grant references on table "public"."system_broadcast_logs" to "anon";

grant select on table "public"."system_broadcast_logs" to "anon";

grant trigger on table "public"."system_broadcast_logs" to "anon";

grant truncate on table "public"."system_broadcast_logs" to "anon";

grant update on table "public"."system_broadcast_logs" to "anon";

grant delete on table "public"."system_broadcast_logs" to "authenticated";

grant insert on table "public"."system_broadcast_logs" to "authenticated";

grant references on table "public"."system_broadcast_logs" to "authenticated";

grant select on table "public"."system_broadcast_logs" to "authenticated";

grant trigger on table "public"."system_broadcast_logs" to "authenticated";

grant truncate on table "public"."system_broadcast_logs" to "authenticated";

grant update on table "public"."system_broadcast_logs" to "authenticated";

grant delete on table "public"."system_broadcast_logs" to "service_role";

grant insert on table "public"."system_broadcast_logs" to "service_role";

grant references on table "public"."system_broadcast_logs" to "service_role";

grant select on table "public"."system_broadcast_logs" to "service_role";

grant trigger on table "public"."system_broadcast_logs" to "service_role";

grant truncate on table "public"."system_broadcast_logs" to "service_role";

grant update on table "public"."system_broadcast_logs" to "service_role";

grant delete on table "public"."system_broadcasts" to "anon";

grant insert on table "public"."system_broadcasts" to "anon";

grant references on table "public"."system_broadcasts" to "anon";

grant select on table "public"."system_broadcasts" to "anon";

grant trigger on table "public"."system_broadcasts" to "anon";

grant truncate on table "public"."system_broadcasts" to "anon";

grant update on table "public"."system_broadcasts" to "anon";

grant delete on table "public"."system_broadcasts" to "authenticated";

grant insert on table "public"."system_broadcasts" to "authenticated";

grant references on table "public"."system_broadcasts" to "authenticated";

grant select on table "public"."system_broadcasts" to "authenticated";

grant trigger on table "public"."system_broadcasts" to "authenticated";

grant truncate on table "public"."system_broadcasts" to "authenticated";

grant update on table "public"."system_broadcasts" to "authenticated";

grant delete on table "public"."system_broadcasts" to "service_role";

grant insert on table "public"."system_broadcasts" to "service_role";

grant references on table "public"."system_broadcasts" to "service_role";

grant select on table "public"."system_broadcasts" to "service_role";

grant trigger on table "public"."system_broadcasts" to "service_role";

grant truncate on table "public"."system_broadcasts" to "service_role";

grant update on table "public"."system_broadcasts" to "service_role";


  create policy "super_admin_view_broadcast_logs"
  on "public"."system_broadcast_logs"
  as permissive
  for select
  to public
using ((EXISTS ( SELECT 1
  ...
  create policy "super_admin_manage_broadcasts"
...
using ((EXISTS ( SELECT 1
  ...
  WHERE ((b.id = booking_extensions.booking_id) AND (c.owner_id = auth.uid())))));
...
using (public.is_admin(auth.uid()));
  ...
  for select
  to public
using ((((is_deleted = false) OR (is_deleted IS NULL)) AND (EXISTS ( SELECT 1
   FROM public.cars
...
using ((EXISTS ( SELECT 1
   FROM public.host_wallets
  WHERE ((host_wallets.id = wallet_transactions.wallet_id) AND (host_wallets.host_id = auth.uid())))));
...
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user_welcome_email();

drop policy "Admins can access all insurance documents" on "storage"."objects";

drop policy "Car owners can delete their car images" on "storage"."objects";

drop policy "Car owners can update their car images" on "storage"."objects";

drop policy "Car owners can upload car images" on "storage"."objects";

drop policy "Users can view message attachments they have access to" on "storage"."objects";
