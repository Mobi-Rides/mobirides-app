-- First, let's clean up duplicate notifications for the same booking
DELETE FROM notifications 
WHERE id NOT IN (
  SELECT MIN(id) 
  FROM notifications 
  GROUP BY user_id, type, related_booking_id, content
);

-- Add new notification types for better flow
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'booking_request_sent';
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'pickup_reminder';
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'return_reminder';
