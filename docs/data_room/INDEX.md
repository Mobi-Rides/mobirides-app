# MobiRides Investor Data Room - INDEX

**Project:** MobiRides - Botswana P2P Car-Sharing Platform  
**Raise:** P700,000 Pre-Seed  
**Last Updated:** April 2026

---

## Navigation

| Section | Folder | Status |
|---------|--------|--------|
| [1. Corporate & Legal](#1-corporate--legal) | `01_corporate_legal/` | ⚠️ |
| [2. Investment](#2-investment) | `02_investment/` | ❌ |
| [3. Financials](#3-financials) | `03_financials/` | ❌ |
| [4. Business](#4-business) | `04_business/` | ✅ |
| [5. Traction](#5-traction) | `05_traction/` | ⚠️ |
| [6. Legal & Contracts](#6-legal--contracts) | `06_legal_contracts/` | ✅ |
| [7. Due Diligence](#7-due-diligence) | `07_due_diligence/` | ⚠️ |

---

## 1. Corporate & Legal

| Document | Status | Priority | Notes |
|----------|--------|----------|-------|
| Company Registration Certificate | ❌ Missing | Critical | Need to generate/acquire |
| Articles of Association | ❌ Missing | Critical | To be drafted |
| Shareholder Agreement | ⚠️ Review | Critical | Draft pending founder signatures |
| Founder Resumes/CVs | ⚠️ Review | Important | Update needed |
| Cap Table | ⚠️ Review | Critical | Update with current equity allocations |
| Board Meeting Minutes | ❌ Missing | Important | Historical records needed |
| IP Assignment Agreements | ❌ Missing | Important | Founder IP transfer docs |

**Action Items:**
- [ ] Obtain company registration certificate
- [ ] Draft/update shareholder agreement
- [ ] Finalize cap table
- [ ] Compile founder resumes

---

## 2. Investment

| Document | Status | Priority | Notes |
|----------|--------|----------|-------|
| Term Sheet Template | ❌ Missing | Critical | Standard pre-seed terms |
| Valuation Methodology | ⚠️ Review | Important | See `docs/Roadmaps*/MobiRides_Valuation_Framework_06-02-2026.md` |
| Offer Letter Template | ❌ Missing | Critical | Prepare for investors |
| SAFEs/Convertible Notes | ❌ Missing | Important | Standard YC/Founder VC docs |
| Investment Agreement | ❌ Missing | Critical | To be used when closing |
| Pro-Rata Rights Documentation | ❌ Missing | Nice-to-have | Future rounds |

**Action Items:**
- [ ] Create term sheet template
- [ ] Prepare SAFE documents
- [ ] Define valuation: P7M pre-money

---

## 3. Financials

| Document | Status | Priority | Notes |
|----------|--------|----------|-------|
| Historical P&L | ❌ Missing | Critical | No revenue yet - pre-revenue |
| Current Bank Statements | ❌ Missing | Critical | Need 3-month statements |
| 12-Month Financial Projections | ⚠️ Review | Critical | See existing model |
| Monthly Cash Flow Statement | ❌ Missing | Critical | Track burn rate |
| Revenue Model | ⚠️ Review | Important | In business plan |
| Operating Expense Budget | ⚠️ Review | Important | Need detailed budget |
| Use of Funds Breakdown | ⚠️ Review | Critical | 18-month budget |

**Action Items:**
- [ ] Create 12-month projections
- [ ] Document use of funds (P700K allocation)
- [ ] Track monthly burn rate

---

## 4. Business

| Document | Status | Path | Priority |
|----------|--------|------|----------|
| Pitch Deck | ✅ Have | `docs/funding/20260424_PRESEED_PITCH_DECK.md` | Critical |
| Executive Summary | ✅ Have | `docs/funding/20260424_PRESEED_EXECUTIVE_SUMMARY.md` | Critical |
| Business Plan | ✅ Have | `docs/Roadmaps*/20260206_MobiRides_Commercialization_GTM_Plan.md` | Important |
| Roadmap 2026 H1 | ✅ Have | `docs/Roadmaps*/20260410_Roadmap_2026_H1.md` | Important |
| Product Roadmap | ✅ Have | `docs/Roadmaps*/ROADMAP.md` | Important |
| Market Analysis | ⚠️ Review | - | Need market size data |
| Competitive Landscape | ⚠️ Review | - | Competitor research needed |
| Customer Discovery Notes | ⚠️ Review | - | User interview summaries |

---

## 5. Traction

| Document | Status | Priority | Notes |
|----------|--------|----------|-------|
| User Metrics Dashboard | ⚠️ Review | Important | Extract from app |
| GMV/Revenue Metrics | ⚠️ Review | Important | Verify current numbers |
| Active Users Count | ⚠️ Review | Important | Need current count |
| Bookings Completed | ⚠️ Review | Important | Track via analytics |
| NPS Score | ❌ Missing | Nice-to-have | Conduct survey |
| User Testimonials | ❌ Missing | Important | Request from users |
| Demo Video | ❌ Missing | Important | Create product demo |
| Product Screenshots | ⚠️ Review | Important | Current UI screenshots |

**Action Items:**
- [ ] Extract user metrics from analytics
- [ ] Create demo video
- [ ] Capture product screenshots

---

## 6. Legal & Contracts

| Document | Status | Path | Priority |
|----------|--------|------|----------|
| Privacy Policy | ✅ Have | `docs/plans/PRIVACY_POLICY.md` | Critical |
| Terms of Service | ⚠️ Review | `docs/Roadmaps*/` | Critical |
| Insurance SLA | ✅ Have | `docs/Roadmaps*/20260319_DAMAGE_PROTECTION_SLA_PAYU.md` | Important |
| Partnership Agreements | ❌ Missing | - | Negotiations pending |
| Driver Agreement T&Cs | ⚠️ Review | - | Need standard agreement |
| API Documentation | ❌ Missing | Nice-to-have | Developer docs |

---

## 7. Due Diligence

| Document | Status | Priority | Notes |
|----------|--------|----------|-------|
| DD Questionnaire Template | ❌ Missing | Critical | Standard investor questions |
| Technical Due Diligence | ⚠️ Review | Important | Code review pending |
| Security Audit Report | ❌ Missing | Important | Third-party audit |
| Data Room Checklist | ⚠️ Review | This file | Update regularly |
| Reference Letters | ❌ Missing | Nice-to-have | Advisor references |
| AdvisorBios | ⚠️ Review | Nice-to-have | Formalize advisor roles |

---

## Summary

| Category | Complete | Review | Missing |
|----------|----------|--------|---------|
| Corporate & Legal | 0 | 4 | 4 |
| Investment | 0 | 1 | 8 |
| Financials | 0 | 3 | 6 |
| Business | 5 | 3 | 0 |
| Traction | 0 | 4 | 4 |
| Legal & Contracts | 2 | 3 | 1 |
| Due Diligence | 0 | 2 | 4 |
| **TOTAL** | **7** | **20** | **27** |

---

## Next Steps

**Immediate Actions (This Week):**
1. ✅ Complete business documents (mostly done)
2. ❌ Create term sheet and investment docs
3. ❌ Build financial projections
4. ⚠️ Finalize corporate documents

**Before Investor Meetings:**
1. Update pitch deck with latest metrics
2. Create demo video
3. Complete financial model
4. Obtain/register company documents

---

## Contact

**Founder:** [Name]  
**Email:** founders@mobirides.co.bw  
**Phone:** +267 XXX XXXX

---

*This data room is maintained for investor due diligence. Last updated: April 2026*